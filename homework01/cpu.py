...
#the class has been altered to include an input variable that would allow the CPU run in batch mode
class CPU(threading.Thread):
    def __init__(self, ram, os, startAddr, debug, batch_mode = False, num=0):
        ...
        self._os = os
        self._debug = debug
        self._startaddr = startAddr #start addr. address where program would be stored
        ...
        # stored.
	# mode is assigned the value batch_mode in constructor
	...
	If the CPU would run in batch mode, the value is set to true, and 
	the registers are cleared before running the program and updating pc addr'''
        if self._mode != True:
            self._run_program()
        else:
            while (self._ram[self._startaddr] != 0):
                self.set_pc(self._ram[self._startaddr])
                self._registers['reg0']
                self._registers['reg1']
                self._registers['reg2']
                self._run_program()
                self._startaddr += 1
    ...
    def handle_call(self, fname):
        self._os.syscall(fname, self._registers['reg0'], self._registers['reg1'], self._registers['reg2'])
